...
public class EditController {
    ...
    public void handleEntabOrDetab(String userSelection){
        ...
        codeArea.replaceSelection(modified);
        //restyle Code Area
        if(codeTabPane.getFileName().endsWith(".asm") || codeTabPane.getFileName().endsWith("s")) {
            MIPSStyle style = new MIPSStyle();
            setStyleSpansNew(codeArea, style);
        }
        else {
...
//        codeArea.setStyleSpans(0, JavaStyle.computeHighlighting(codeArea.getText()));
    ...
    /**
     * Helper method for setStyleSpans
     *
     * @param codeArea the current codeArea
     * @param style the codeArea style
     */
    private void setStyleSpansNew(CodeArea codeArea, Style style) {
        codeArea.setStyleSpans(0, style.computeHighlighting(codeArea.getText()));
    }
